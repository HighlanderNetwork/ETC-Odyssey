...
contract ETCOdyssey
{
  ...
  function initShipToken(uint ref, uint amount) public payable{//TODO
    require(amount >= minimumSpendETC);
    require(playerList[msg.sender].lastUpdate == 0);
    require(amount >= darkMatterExchangeRateToken);
    //require(ERC20(tokenAddress).balanceOf(msg.sender) >= amount);
    //(ERC20(tokenAddress).allowance(msg.sender, address(this)) >= amount);
    playerList[msg.sender].lastUpdate = now;
    //ERC20(tokenAddress).transferFrom(msg.sender, address(this), amount);
    uint earnedMatter = amount / darkMatterExchangeRate;
    playerList[msg.sender].darkMatter += earnedMatter;
    if(referralList[ref] != address(0)){
      playerList[referralList[ref]].referralCount += 1;
      referrer[msg.sender] = referralList[ref];
  ...
  function buyDarkMatterToken(uint amount) public payable{//TODO
    require(playerList[msg.sender].lastUpdate != 0);
    require(amount >= darkMatterExchangeRateToken);
    //require(ERC20(tokenAddress).balanceOf(msg.sender) >= amount);
    //require(ERC20(tokenAddress).allowance(msg.sender, address(this)) >= amount);
    updateStats(msg.sender);
    //ERC20(tokenAddress).transferFrom(msg.sender, address(this), amount);
    uint earnedMatter = amount / darkMatterExchangeRateToken;
    playerList[msg.sender].darkMatter += earnedMatter;
    if(referrer[msg.sender] != address(0)){
      playerList[referrer[msg.sender]].darkMatter += earnedMatter / 100;
    }
